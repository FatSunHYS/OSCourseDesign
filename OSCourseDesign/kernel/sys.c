...
{
	if( syscall_number < __NR_end )
	{
		return syscall_number_counter[ syscall_number ];
	}
...
asmlinkage long sys_resetsyscallcounter()
{
	int i;
	for( i = 0; i < __NR_end; ++i )
	{
		syscall_number_counter[ i ] = 0;
	}
